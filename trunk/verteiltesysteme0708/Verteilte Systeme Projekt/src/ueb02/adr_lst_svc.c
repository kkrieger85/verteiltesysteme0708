	...
	SVCXPRT *transp;
{
	...
	case ADR_LST_AUTH:
		xdr_argument = xdr_adr_lst_auth_data;
		xdr_result = xdr_int;
		local = (char *(*)()) adr_lst_auth_1_svc;
		break;
	...
	case ADR_LST_LOGOFF:
		...
		xdr_result = xdr_int;
		local = (char *(*)()) adr_lst_logoff_1_svc;
		break;
...
//#endif
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_DGRAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_UDP;
		if (!svc_register(transp, ADR_LST, ONE, adr_lst_1, proto)) {
			_msgout("unable to register (ADR_LST, ONE, udp).");
			exit(1);
		}
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_STREAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_TCP;
		if (!svc_register(transp, ADR_LST, ONE, adr_lst_1, proto)) {
			_msgout("unable to register (ADR_LST, ONE, tcp).");
			exit(1);
		}
	}
